...
const {
  ...
  expiredCondition,
  isDbSupportJSON
} = require('knex-mysql-session/lib/utils');
...
function dbCleanup(store, interval, KnexStore, errorCallback) {
  return store.ready
    .then(() => {
        return store.knex.raw('call users.pruneExpiredSessions(?);', [dateAsISO(store.knex)]);
    })
    .catch(errorCallback)
    .finally(() => {
...
module.exports = function (connect) {
  ...
  function KnexStore(options = {}) {
    ...
    self.knex = options.knex
      || knexFactory({
        ...
        // debug: true,
        connection: {
          filename: 'knex-mysql-session.sqlite',
        },
      });
  ...
  KnexStore.prototype.set = function (sid, sessObject, fn) {
    ...
    if (isMySQL(self.knex)) {
      ...
      return self.ready.then(() => resolve(self.knex.raw(
          'call addSession(?,?,?)', [sid, dbDate, sess]
      )).asCallback(fn));
    }
  ...
  KnexStore.prototype.touch = function (sid, sess, fn) {
    if (sess && sess.cookie && sess.cookie.expires) {
      ...
      return resolve(this.knex.raw(
          'call touchSession(?,?)',
          [dateAsISO(this.knex), sid]
      )).asCallback(fn);
    }
   ...
   * @api public
   */
  KnexStore.prototype.destroy = function (sid, fn) { // TODO
    const self = this;
    return self.ready.then(() => resolve(self.knex
      .del()
      .from(self.tablename)
   ...
   * @api public
   */
  KnexStore.prototype.clear = function (fn) { // TODO
    const self = this;
    return self.ready.then(() => resolve(self.knex
      .del()
      .from(self.tablename))
